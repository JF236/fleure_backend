import { GetIdsByBundle } from '../src/endpoints/getIdsByBundle';

describe('GetIdsByBundle', () => {
  ...
  it('should return list of message ids when found', async () => {
    ...
    const getIdsByBundle = new GetIdsByBundle();
    const response = await getIdsByBundle.handle(requestMock, mockEnv, contextMock, dataMock);
    ...
    expect(mockEnv.DB.bind).toHaveBeenCalledWith(dataMock.params.bundle_id); // Ensure correct binding of bundle_id
